...
;(()=>{
  ...
  function headers(){const t=token();return {apikey:KEY,Authorization:'Bearer '+t,'Content-Type':'application/json'}}
  async function rpc(name,payload={}){
    const t=token();if(!t)throw new Error('هویت مهمان هنوز آماده نیست.');
  ...
  async function waitForToken(){
    for(let i=0;i<36;i++){if(token())return token();await sleep(250)}
    return null;
  }

  // Keep polling only as a safety net. Realtime becomes the primary update path.
  ...
  function ensureSyncPill(){
    let node=$('#shared-sync-state');
    if(node)return node;
    const top=$('.top-state');if(!top)return null;
    node=document.createElement('span');node.id='shared-sync-state';node.className='mode-pill';node.textContent='Server Sync';
    top.append(node);return node;
  }
  function syncState(label,state='idle'){
    const node=ensureSyncPill();if(!node)return;
  ...
  function ensureStyles(){
    ...
    const style=document.createElement('style');style.id='kaykha-shared-engine-style';style.textContent=`
      ...
      #practice-next-phase{border:1px solid #c9a45d88;background:#6d4d20;color:#f4dfa2;border-radius:9px;padding:8px 11px;font:inherit;font-size:10px;cursor:pointer}
      #practice-next-phase[hidden]{display:none!important}.shared-engine-note{color:#7fc6bc!important}
  ...
  function toast(message){
    const old=$('.shared-result-toast');old?.remove();
    const node=document.createElement('div');node.className='shared-result-toast';node.textContent=message;document.body.append(node);setTimeout(()=>node.remove(),3700);
  }
  ...
  function visualEffect(effect){
    ...
    const button=[...document.querySelectorAll('#territories button')].find(b=>b.querySelector('b')?.textContent?.trim()===name);
    if(!button)return;
  ...
  async function loadManifest(){
    try{manifest=await rpc('get_kaykha_action_manifest',{});applyManifest()}catch(_){}
  }
  function applyManifest(){
    const order=$('#orders .active')?.dataset.order;if(!manifest||!order||!manifest[order])return;
    const m=manifest[order];
    const summary=$('#order-intel-summary'),gain=$('#order-intel-gain'),risk=$('#order-intel-risk'),dawn=$('#order-intel-dawn');
  ...
  function syncAiPanel(){
    ...
    let next=$('#practice-next-phase');
    if(!next){next=document.createElement('button');next.id='practice-next-phase';next.type='button';next.textContent='ورود به فاز فرمان';panel.append(next)}
    next.hidden=phaseCache!=='negotiation';
  }

  async function readGameMeta(gameId){
    if(!gameId)return null;
    const rows=await table('kaykha_games?id=eq.'+encodeURIComponent(gameId)+'&select=id,status,phase,round_no,is_practice,practice_difficulty&limit=1');
    return Array.isArray(rows)?rows[0]||null:null;
  }
  async function ensurePracticeGame(){
    if(!isPracticeUrl())return false;
    const t=await waitForToken();if(!t){syncState('هویت آماده نیست','bad');return false}
    let id=localStorage.getItem(PRACTICE_KEY),meta=null;
    if(id){try{meta=await readGameMeta(id)}catch(_){meta=null}}
    if(!meta||!meta.is_practice||meta.status!=='active'){
      syncState('ساخت Practice…','sync');
      const rows=await rpc('create_kaykha_practice_game',{
        p_display_name:($('#commander-name')?.value||'فرمانده تمرین').trim()||'فرمانده تمرین',
        p_house_id:textOf('#faction','هخامنشیان'),
        p_persona_key:textOf('#persona','اسپهبد · پاسدار'),
        p_difficulty:difficulty()
      });
      const row=Array.isArray(rows)?rows[0]:rows;
      id=row?.game_id;if(!id)throw new Error('Practice ساخته نشد.');
      localStorage.setItem(PRACTICE_KEY,id);localStorage.setItem(GAME_KEY,id);
      sessionStorage.setItem('kaykha.practice-engine-reload','1');
    ...
    const current=localStorage.getItem(GAME_KEY);
    if(current!==id){localStorage.setItem(GAME_KEY,id);location.reload();return true}
  ...
  function scheduleRefresh(){
    clearTimeout(realtimeTimer);realtimeTimer=setTimeout(()=>{syncState('در حال Sync…','sync');window.dispatchEvent(new Event('focus'));setTimeout(()=>syncState('Realtime متصل','live'),450)},120);
  }
  async function bindRealtime(gameId){
    ...
    try{
      ...
      realtimeClient=lib.createClient(URL,KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false},global:{headers:{Authorization:'Bearer '+token()}}});
      ...
      ['kaykha_games','kaykha_members','kaykha_territories','kaykha_events','kaykha_contracts','kaykha_loans'].forEach(tableName=>{
        realtimeChannel.on('postgres_changes',{event:'*',schema:'public',table:tableName,filter:'game_id=eq.'+gameId},payload=>{
          if(tableName==='kaykha_games'&&payload.new){phaseCache=payload.new.phase||phaseCache;practiceCache=Boolean(payload.new.is_practice??practiceCache);syncAiPanel()}
          scheduleRefresh();
        });
      });
      ...
      realtimeChannel.subscribe(status=>{
        if(status==='SUBSCRIBED')syncState('Realtime متصل','live');
        else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT')syncState('Fallback Sync','bad');
        else syncState('در حال اتصال…','sync');
      });
  ...
  async function refreshMeta(){
    ...
    try{const meta=await readGameMeta(gameId);if(meta){phaseCache=meta.phase;practiceCache=Boolean(meta.is_practice);syncAiPanel()}}catch(_){}
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if(ai&&practiceCache){
      ...
      const d=ai.dataset.aiMode;if(!['easy','hard','mastermind'].includes(d))return;
      localStorage.setItem(AI_KEY,d);syncAiPanel();
      const gameId=localStorage.getItem(GAME_KEY);
      syncState('ثبت سطح AI…','sync');
      rpc('set_kaykha_practice_difficulty',{p_game_id:gameId,p_difficulty:d}).then(()=>{syncState('Realtime متصل','live');toast('سطح AI روی «'+({easy:'آسان',hard:'سخت',mastermind:'ذهن برتر'})[d]+'» تنظیم شد.')}).catch(error=>{syncState('خطای Sync','bad');toast(error.message)});
      return;
    }
    if(event.target.closest('#practice-next-phase')&&practiceCache){
      event.preventDefault();const gameId=localStorage.getItem(GAME_KEY);syncState('باز کردن فرمان‌ها…','sync');rpc('open_kaykha_orders',{p_game_id:gameId}).then(()=>{phaseCache='orders';syncAiPanel();scheduleRefresh()}).catch(error=>toast(error.message));return;
    }
  ...
  window.addEventListener('kaykha:lobby-success',event=>{
    const d=event.detail||{};if(!d.gameId)return;
    const url=new URL(location.href);url.searchParams.set('mode','online');history.replaceState({},'',url);
    practiceCache=false;localStorage.setItem(GAME_KEY,d.gameId);bindRealtime(d.gameId);
  });
  window.addEventListener('kaykha:identity',()=>{setTimeout(()=>{refreshMeta();syncAiPanel()},0)});
  window.addEventListener('kaykha:shared-effect',event=>visualEffect(event.detail));

  async function boot(){
    ensureStyles();ensureSyncPill();syncState('آماده‌سازی موتور…','sync');
    try{
      const reloaded=await ensurePracticeGame();if(reloaded)return;
      await waitForToken();
      const gameId=localStorage.getItem(GAME_KEY);
      if(gameId){await refreshMeta();await Promise.allSettled([loadManifest(),bindRealtime(gameId)])}
      else syncState('بدون تالار','idle');
