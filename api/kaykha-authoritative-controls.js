module.exports = function asset(_request, response) {
  function authorityController() {
    ...
    function installStyles() {
      ...
      style.textContent = `
        ...
        #kaykha-cause-effect{margin:8px 0 12px;padding:10px;border:1px solid rgba(201,164,93,.28);background:rgba(5,14,20,.72);border-radius:12px}
        #kaykha-cause-effect header{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:8px}
        #kaykha-cause-effect header b{color:#ead28a;font-size:10px}#kaykha-cause-effect header small{color:#78bdb2;font-size:7px}
        .ce-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.ce-grid>div{min-width:0;padding:8px;border:1px solid rgba(255,255,255,.07);background:#0003}
        .ce-grid small{display:block;color:#809596;font-size:7px;margin-bottom:3px}.ce-grid b{display:block;color:#c3d0ce;font-size:9px;line-height:1.7}
    ...
    function activeOrder() {
      return $('#orders [data-order].active')?.dataset.order || $('#orders [data-order]')?.dataset.order || null;
    }

    function syncOrder(order = activeOrder()) {
      if (!manifest || !order || !manifest[order]) return;
      const rule = manifest[order];
      const button = $('#orders [data-order="' + order + '"]');
      const label = button?.textContent?.trim() || order;
    ...
    function bind() {
      ...
      window.addEventListener('kaykha:server-sync-request', () => {
    ...
    function boot() {
      ...
      bind();
      loadManifest();
      document.documentElement.dataset.kaykhaAuthority = 'rules-only-v3';
    }
  ...
  response.end(';(' + authorityController.toString() + ')();');
};
