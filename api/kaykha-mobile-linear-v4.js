    ...
    function installStyles() {
      ...
      style.textContent = `
        ...
        @media (max-width:1024px),(pointer:coarse) and (max-device-width:1024px){
          ...
          #kx-mobile-scrim{display:block;position:fixed;z-index:940;top:0;left:0;right:0;bottom:calc(72px + env(safe-area-inset-bottom));border:0;background:#030201b8;backdrop-filter:blur(3px);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .18s ease,visibility .18s ease}#kx-mobile-scrim.show{opacity:1;visibility:visible;pointer-events:auto}
          #kx-mobile-more-sheet,#kx-causal-sheet{display:block;position:fixed;z-index:960;left:8px;right:8px;bottom:calc(84px + env(safe-area-inset-bottom));max-height:58dvh;overflow:auto;overscroll-behavior:contain;padding:12px;border:1px solid var(--kx-line);border-radius:18px 18px 12px 12px;background:#1e160ff8;box-shadow:0 -20px 60px #000b;transform:translateY(calc(100% + 100px));opacity:0;visibility:hidden;pointer-events:none;transition:transform .2s ease,opacity .18s ease,visibility .18s ease}#kx-mobile-more-sheet.show,#kx-causal-sheet.show{transform:none;opacity:1;visibility:visible;pointer-events:auto}.kx-sheet-handle{width:42px;height:4px;border-radius:99px;background:#7a6954;margin:0 auto 10px}
